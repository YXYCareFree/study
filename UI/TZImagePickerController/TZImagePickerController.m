...
- (void)showProgressHUD {
    if (!_progressHUD) {
        ...
        _HUDLable.text = LocatizedStringForkey(@"lib_photopicker_processing", @"正在处理...");
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.childViewControllers.count > 0) {
        /*
        UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(3, 0, 60, 44)];
        ...
        [backButton setTitle:LocatizedStringForkey(@"lib_photopicker_back", @"返回") forState:UIControlStateNormal];
        backButton.titleLabel.font = [UIFont systemFontOfSize:15];
        [backButton addTarget:self action:@selector(popViewControllerAnimated:) forControlEvents:UIControlEventTouchUpInside];
        */
        //[[UIBarButtonItem alloc] initWithCustomView:backButton];
        UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:LocatizedStringForkey(@"lib_photopicker_back", @"返回") style:UIBarButtonItemStylePlain target:self action:@selector(popViewControllerAnimated:)];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    self.navigationItem.title = LocatizedStringForkey(@"lib_photopicker_photo", @"照片");
